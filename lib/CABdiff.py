...
class CABDiffAlgorithm():
	...
	def decide(self,pool_articles,userID,rwdu,ThetaStar):
		...
		for k in pool_articles:
			...
			for j in range(len(self.users)):
				...
				if(j!=userID):
					...
					if (abs(compare)<=CBJ+diffR)&(self.users[j].CoTheta!=temp).all()&(diffR<=CBJ):
	...
	def updateParameters(self, articlePicked, click,userID,gamma):
		...
		with open(self.filenameWritePara, 'a+') as f:
			if(clusterNow!=[]):
				...
				for i in range(len(clusterNow)):
					f.write('\t'+str(clusterNow[i]))
				f.write('\n')
